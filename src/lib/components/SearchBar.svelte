<script lang="ts">
  ...
  const getLocation = async (locationName: string) => {
    const response = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(
      `${locationName} 02138`
    )}`);
  ...
  const searchOrigin = () => getLocation(originLocationName).then(location => {
  ...
  const searchDestination = () => getLocation(destinationLocationName).then(location => {
...
<div class="area">
  <div class="inputs">
    <div class="aligned" bind:this={originInput}>    
      ...
      />
      {#if modifiedOrigin}
        <IconButton class="material-icons search" size="mini" on:click={searchOrigin}>search</IconButton>
      {/if}
    </div>
    <div class="aligned" bind:this={destinationInput}>    
      ...
      {#if modifiedDestination}
        <IconButton class="material-icons search" size="mini" on:click={searchDestination}>search</IconButton>
      {/if}
    </div>
  </div>
</div>
